...
const { getCacheBufferFromMinio, getBufferThroughMinio } = require("./basic");
...
async function getNativeTileBuffer(options) {
  const cacheBuffer = await getCacheBufferFromMinio(options);
  if (cacheBuffer) {
    return cacheBuffer;
  } else {
    return await getBufferThroughMinio(options);
  }
}
